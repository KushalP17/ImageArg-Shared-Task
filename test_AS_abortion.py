...
def agent_forest(tweet_text, tweet_id, stance, num_agents=3):
    ...
    input_string = f"""You are a sentiment analysis bot who takes a tweet about Abortion as an input and outputs "support" if the tweet supports Abortion and "oppose" if the tweet opposes Abortion. 
                            You are an unbiased third party only classifying these tweets. 
                            Please be aware of hashtags associated with each side, such as #prolife, #maga, #trump, #donaldtrump, #republican, #conservative, #christian, #murder, #voteforlife, #faith, #bornaliveact,  etc. that are associated with opposing Abortion, 
                            and hashtags like #prochoice, #democrat, #womensrights, #womenschoice, #women, #liberal, #mybodymychoice, #bodilyautonomy, #reproductiverights, #righttochoose, #abortionpill, #SaveRoe, #RoeVWade, #WeCan, etc. that are associated with supporting Abortion. 
