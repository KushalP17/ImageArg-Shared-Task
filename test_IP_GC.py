...
for index, row in df.iterrows():
    ...
    #     image_text = pytesseract.image_to_string(thresh)
        ...
        input_string_p = f"""You are a sentiment analysis bot that classifies an image based on topic content. If the image is unrelated to Gun Control or no image is given, simply respond with "null". If it is related to Gun Control, classify the content to distinguish whether it supports or opposes Gun Control.

                        Please be aware of certain sentiments and slogans such as Second Amendment Rights, Christian, conservative, republican, etc. which opposes Gun Control and End Gun Violence, Gun Control Now, liberal, democrat, women's choice, etc. which supports Gun Control. 
